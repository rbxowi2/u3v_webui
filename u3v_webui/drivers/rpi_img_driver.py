"""
drivers/rpi_img_driver.py — Raspberry Pi still-mode camera driver via picamera2.
...
Multi-camera
------------
Same as rpi_driver: enumerates all cameras reported by global_camera_info().
Device IDs use the prefix "rpi-img:" to avoid collisions with the video driver.
"""
...
class RPiImgDriver(CameraDriver):
    """
    ...
    def scan_devices() -> list:
        """Enumerate available Pi CSI cameras; return them as still-mode devices."""
        try:
        ...
        for info in cam_infos:
            ...
            result.append({
                "device_id": f"rpi-img:{num}",
                "model":     model,
                "serial":    info.get("Id", f"rpi-img{num}"),
                "label":     f"{model} (CSI cam{num} img)",
            })
        return result
    ...
    def open(self, device_id: Optional[str] = None) -> dict:
        ...
        if width > 0 and height > 0:
            size_kw["size"] = (width, height)

        cfg = cam.create_still_configuration(
            main={"format": "RGB888", **size_kw},
            controls=controls,
            buffer_count=2,
        )
        cam.configure(cfg)
        ...
        return {
            "model":  model,
            "serial": f"csi-img{self._cam_num}",
            "width":  actual_w,
            "height": actual_h,
            **bounds,
        }
    ...
    def query_native_modes(self) -> list:
        ...
        try:
            ...
            for m in self._cam.sensor_modes:
                ...
                if (w, h) not in seen:
                    seen.add((w, h))
                    modes.append({"width": w, "height": h})
            modes.sort(key=lambda e: -(e["width"] * e["height"]))
            return modes
        except Exception:
    ...
    def _apply_pending(self):
        ...
        if controls:
            try:
                ...
                print(f"[RPiImgDriver] set_controls failed: {e}", flush=True)
